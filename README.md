...
cargo build
```
5.	Running the code does nothing since there is no code in main. This would normally be done with...
```
cargo run
```
6.	Run all tests cases with output using...
```
cargo test -- --nocapture
```
7.	Compile all documentation with...
```
cargo doc
```
...
fn test_channel()
```
In Rust, all functions marked with the test macro are run as tests when you run...
```rust
cargo test
```
This testing tests all the basic functions.
